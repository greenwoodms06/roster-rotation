...
- **Global max periods per player** -- cap how many total periods any player can play, regardless of position. Use case: "No player plays more than 3 of 4 quarters." Works with any sport. Treated as a hard cap — the engine relaxes the sub cap before exceeding this value. When you add a period after the fact and every player is already at the cap, the app asks if you want to raise the cap by 1 so the new period can be filled (rather than failing silently).
- **Max subs per break** -- cap how many players can be subbed out between consecutive periods. Use case: "I've got 14 kids and only want to swap 2 at a time so the game keeps flowing." Defaults to Any. Soft constraint: may force a player past their equal-time share to keep the roster stable, and relaxes automatically when the hold-over pool exhausts against the Max Periods cap. A post-pass (2-swap repair) then tries to eliminate remaining sub-cap violations without degrading any player's rest/fatigue profile — it tolerates at most a +1 extension to any player's longest play or sit streak, so K=2 is honored in most cases while still preventing 4+ consecutive-period stretches. When it forces overflow, the player with the lowest season-ratio is chosen first so overflow rotates across games.

**Constraint precedence** (when over-constrained): fully staff every period → honor Max Periods → equal playing time → fatigue / rest spread (post-repair can't worsen streaks by more than +1) → relax Max Subs as needed → relax Max Periods only as a last resort if no valid plan exists.
